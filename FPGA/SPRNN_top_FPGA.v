...
module SPRNN_Top#(
    ...
    // 프레임별 크기
    localparam FRAME0_SIZE = 16'd1250;
    localparam FRAME1_SIZE = 16'd2500;
    localparam FRAME2_SIZE = 16'd1250;
    ...
    reg     [7:0]           cnt_row, cnt_row_n;
    reg     [7:0]           cnt_column, cnt_column_n;
    ...
    assign memA_wenb = (send) ? mem_rd_wenb : ((initial_SRAMw_done & start) ? memA_wenb_o : (start ? 0 : 1));
   ...
   /////////////////////// data_send////////////////////////
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            valid        <= 0;
            image_data   <= 8'd0;
            total_bytes  <= FRAME0_SIZE;
        end else begin
            if (send && !busy) begin
                valid <= 1;
                image_data <= memA_qout[127:120];
            end else begin
                valid <= 0;
            end
            // 프레임 전송 완료되면 다음 프레임 설정
            if (tx_done) begin
                byte_counter <= 0;
                case (frame_state)
                    0: begin
                        total_bytes <= FRAME1_SIZE;
                        frame_state <= 1;
                    end
                    1: begin
                        total_bytes <= FRAME2_SIZE;
                        frame_state <= 2;
                    end
                    2: begin
                        total_bytes <= FRAME0_SIZE;
    ...
    always @ (*) begin
        ...
        cnt_row_n           = cnt_row;
        cnt_column_n        = cnt_column;
        color_n             = color;
        case ( color )
            red: begin
                if (cnt_column <= 'd24) begin
                    if (cnt_row < 'd49) begin
                        mem_rd_cenb     = 0;
                        ...
                        mem_rd_cenb     = 0;
                        mem_rd_addr_n   = mem_rd_addr + 'd53;
                        cnt_row_n       = 'd0;
                        cnt_column_n    = cnt_column + 1;
                        if (cnt_column == 'd24) begin
                            mem_rd_cenb     = 0;
                            mem_rd_addr_n   = 'd103 + 'd2652;
                            color_n         = green;
                            cnt_row_n       = 'd0;
                            cnt_column_n    = 'd0;
                        end
