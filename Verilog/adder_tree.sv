  ...
  generate
    for (grp = 0; grp < NUM_GROUPS; grp = grp + 1) begin : GEN_STAGE1
      assign stage1_comb[grp] =
           in_flat[(grp*GROUP_SIZE + 0)*DATA_WIDTH +: DATA_WIDTH]
         + in_flat[(grp*GROUP_SIZE + 1)*DATA_WIDTH +: DATA_WIDTH]
         + in_flat[(grp*GROUP_SIZE + 2)*DATA_WIDTH +: DATA_WIDTH]
         + in_flat[(grp*GROUP_SIZE + 3)*DATA_WIDTH +: DATA_WIDTH];
    end
  endgenerate
