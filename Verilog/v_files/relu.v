module bias_relu #(
    ...
    parameter WIDTH_IN_DATA         = 24, //WIDTH_BITWIDTH * 2 + $clog2(NUM_ACCUMULATE) + $clog2(NUM_ADDER_TREE_INPUTS)
    ...
    if (relu_en) begin
        case (layer_state)
            3'd1: begin
                ...
                clipped = (bias_added < 0) ? 0 : (bias_added > 4064) ? 4064 : bias_added;
                data_out = clipped[12:5];
                relu_done = 1;
            end
            ...
            3'd6: begin
                ...
                bias_ext = {{(WIDTH_BIAS-WIDTH_BITWIDTH){bias[WIDTH_BITWIDTH-1]}}, bias <<<5};
                bias_added = data_extended + bias_ext;
                clipped = (bias_added < -4096) ? 0 : (bias_added > 4064) ? 4064 : bias_added;
                data_out = clipped[12:5];
                relu_done = 1;
            end
