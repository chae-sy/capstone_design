module bias_relu #(
    parameter NUM_CHNL       = 16,
    parameter NUM_ACCUMULATE = 9,
    parameter NUM_ADDER_TREE_INPUTS = 16,
    parameter WIDTH_BITWIDTH = 8,
    parameter WIDTH_IN_DATA = WIDTH_BITWIDTH *2 + $clog2(NUM_ACCUMULATE) + $clog2(NUM_ADDER_TREE_INPUTS), // 16+4+4
    parameter WIDTH_BIAS = 32,
    parameter WIDTH_OUT_DATA = 8,
    
    parameter RELU_MAX_VAL = 6,
    
    parameter WIDTH_L1_IN_IL = 9,
    parameter WIDTH_L1_OUT_IL = 2,
    
    parameter WIDTH_L2_IN_IL = 9,
    parameter WIDTH_L2_OUT_IL = 2,
    
    parameter WIDTH_L3_IN_IL = 9,
    parameter WIDTH_L3_OUT_IL = 2,
    
    parameter WIDTH_L4_IN_IL = 9,
    parameter WIDTH_L4_OUT_IL = 2,
     
    parameter WIDTH_L5_IN_IL = 9,
    parameter WIDTH_L5_OUT_IL = 2,
    
    parameter WIDTH_L6_IN_IL = 9,
    parameter WIDTH_L6_OUT_IL = 2,
      
)
(
    input   wire                                   relu_en,
    output  reg                                    relu_done,
    input   wire [2:0]                             layer_state,
    input   wire  signed [WIDTH_IN_DATA-1:0]       data_in,
    input   wire signed [WIDTH_BIAS-1:0]           bias,
    output  reg [WIDTH_OUT_DATA-1:0]               data_out
    );
    
  
  localparam WIDTH_L1_IN_FL = WIDTH_IN_DATA-WIDTH_L1_IN_IL-1;
  localparam WIDTH_L1_OUT_FL = WIDTH_OUT_DATA-WIDTH_L1_OUT_IL-1;

  localparam WIDTH_L2_IN_FL = WIDTH_IN_DATA-WIDTH_L2_IN_IL-1;
  localparam WIDTH_L2_OUT_FL = WIDTH_OUT_DATA-WIDTH_L2_OUT_IL-1;

  localparam WIDTH_L3_IN_FL = WIDTH_IN_DATA-WIDTH_L3_IN_IL-1;
  localparam WIDTH_L3_OUT_FL = WIDTH_OUT_DATA-WIDTH_L3_OUT_IL-1;

  localparam WIDTH_L4_IN_FL = WIDTH_IN_DATA-WIDTH_L4_IN_IL-1;
  localparam WIDTH_L4_OUT_FL = WIDTH_OUT_DATA-WIDTH_L4_OUT_IL-1;

  localparam WIDTH_L5_IN_FL = WIDTH_IN_DATA-WIDTH_L5_IN_IL-1;
  localparam WIDTH_L5_OUT_FL = WIDTH_OUT_DATA-WIDTH_L5_OUT_IL-1;

  localparam WIDTH_L6_IN_FL = WIDTH_IN_DATA-WIDTH_L6_IN_IL-1;
  localparam WIDTH_L6_OUT_FL = WIDTH_OUT_DATA-WIDTH_L6_OUT_IL-1;


  // bias를 더한 후 RELU(또는 no_relu) 처리. 동시에 relu_done 신호를 생성.
  wire signed [WIDTH_BIAS-1:0] data_extended = 
        { { WIDTH_BIAS-WIDTH_IN_DATA { data_in[WIDTH_IN_DATA-1] } },
          data_in
        } << (WIDTH_BIAS - WIDTH_IN_DATA);

  reg signed [WIDTH_BIAS-1:0] bias_added;

  always @(*) begin
    // 기본값: 연산이 없거나 레이어 상태가 해당하지 않으면 data_out = 0, relu_done = 0
    data_out   = {WIDTH_OUT_DATA{1'b0}};
    relu_done  = 1'b0;
    bias_added = {WIDTH_BIAS{1'b0}};

    if (relu_en) begin
      case (layer_state)
        3'b001: begin
          bias_added = data_extended + bias;
          // 음수라면 zero-point 출력
          if (bias_added[WIDTH_BIAS-1])
            data_out = {WIDTH_OUT_DATA{1'b0}};
          // Q값보다 크다면 클램핑
          else if (bias_added > RELU_MAX_VAL*(2**(WIDTH_L1_NORM_FL)))
            data_out = RELU_MAX_VAL <<< WIDTH_L1_O_FL;
          else
            data_out = {1'b0, bias_added[WIDTH_L1_NORM_FL+WIDTH_L1_O_IL-1:WIDTH_L1_NORM_FL], 
                        bias_added[WIDTH_L1_NORM_FL-1:WIDTH_L1_NORM_FL-WIDTH_L1_O_FL]};
          relu_done = 1'b1;  // 연산 완료 신호
        end

        3'd2: begin
          bias_added = data_extended + bias;
          // 음수라면 zero-point 출력
          if (bias_added[WIDTH_BIAS-1])
            data_out = {WIDTH_OUT_DATA{1'b0}};
          // Q값보다 크다면 클램핑
          else if (bias_added > RELU_MAX_VAL*(2**(WIDTH_L2_NORM_FL)))
            data_out = RELU_MAX_VAL <<< WIDTH_L2_O_FL;
          else
            data_out = {1'b0, bias_added[WIDTH_L2_NORM_FL+WIDTH_L2_O_IL-1:WIDTH_L2_NORM_FL], 
                        bias_added[WIDTH_L2_NORM_FL-1:WIDTH_L2_NORM_FL-WIDTH_L2_O_FL]};
          relu_done = 1'b1;  // 연산 완료 신호
        end

        3'd3: begin
          bias_added = data_extended + bias;
          // 음수라면 zero-point 출력
          if (bias_added[WIDTH_BIAS-1])
            data_out = {WIDTH_OUT_DATA{1'b0}};
          // Q값보다 크다면 클램핑
          else if (bias_added > RELU_MAX_VAL*(2**(WIDTH_L3_NORM_FL)))
            data_out = RELU_MAX_VAL <<< WIDTH_L3_O_FL;
          else
            data_out = {1'b0, bias_added[WIDTH_L3_NORM_FL+WIDTH_L3_O_IL-1:WIDTH_L3_NORM_FL], 
                        bias_added[WIDTH_L3_NORM_FL-1:WIDTH_L3_NORM_FL-WIDTH_L3_O_FL]};
          relu_done = 1'b1;  // 연산 완료 신호
        end

        3'd4: begin // maxpool 모드: bias 추가 없이 data_extended만 사용
          // 음수라면 zero-point 출력
          if (data_extended[WIDTH_BIAS-1])
            data_out = {WIDTH_OUT_DATA{1'b0}};
          // Q값보다 크다면 클램핑
          else if (data_extended > RELU_MAX_VAL*(2**(WIDTH_L3_NORM_FL)))
            data_out = RELU_MAX_VAL <<< WIDTH_L3_O_FL;
          else
            data_out = {1'b0, data_extended[WIDTH_L3_NORM_FL+WIDTH_L3_O_IL-1:WIDTH_L3_NORM_FL], 
                        data_extended[WIDTH_L3_NORM_FL-1:WIDTH_L3_NORM_FL-WIDTH_L3_O_FL]};
          relu_done = 1'b1;  // 연산 완료 신호
        end

        3'd5: begin 
          bias_added = data_extended + bias;
          // 음수라면 zero-point 출력
          if (bias_added[WIDTH_BIAS-1])
            data_out = {WIDTH_OUT_DATA{1'b0}};
          // Q값보다 크다면 클램핑
          else if (bias_added > RELU_MAX_VAL*(2**(WIDTH_L5_NORM_FL)))
            data_out = RELU_MAX_VAL <<< WIDTH_L5_O_FL;
          else
            data_out = {1'b0, bias_added[WIDTH_L5_NORM_FL+WIDTH_L5_O_IL-1:WIDTH_L5_NORM_FL], 
                        bias_added[WIDTH_L5_NORM_FL-1:WIDTH_L5_NORM_FL-WIDTH_L5_O_FL]};
          relu_done = 1'b1;  // 연산 완료 신호
        end

        3'd6: begin // no_relu 모드: 양수/음수 그대로 양자화만 수행
          bias_added = data_extended + bias;
          if(bias_added[WIDTH_NORM_O_DATA-1]) begin
                        if(bias_added < -(2**(WIDTH_L6_O_IL+WIDTH_L6_NORM_FL))) layer_out = 8'b10000000;
                        else if (WIDTH_L6_O_IL == 0) layer_out = {1'b0, bias_added[WIDTH_L6_NORM_FL-1:WIDTH_L6_NORM_FL-WIDTH_L6_O_FL]};
                        else layer_out = {1'b0, bias_added[WIDTH_L6_NORM_FL+WIDTH_L6_O_IL-1:WIDTH_L6_NORM_FL], 
                                      bias_added[WIDTH_L6_NORM_FL-1:WIDTH_L6_NORM_FL-WIDTH_L6_O_FL]};
                    end
                    else begin
                        if(bias_added > 2**(WIDTH_L6_O_IL+WIDTH_L6_NORM_FL)-1) layer_out = 8'b01111111;
                        else if (WIDTH_L6_O_IL == 0) layer_out = {1'b0, bias_added[WIDTH_L6_NORM_FL-1:WIDTH_L6_NORM_FL-WIDTH_L6_O_FL]};
                        else layer_out = {1'b0, bias_added[WIDTH_L6_NORM_FL+WIDTH_L6_O_IL-1:WIDTH_L6_NORM_FL], 
                                      bias_added[WIDTH_L6_NORM_FL-1:WIDTH_L6_NORM_FL-WIDTH_L6_O_FL]};
                    end
          relu_done = 1'b1;
        end

        default: begin
          data_out  = {WIDTH_OUT_DATA{1'b0}};
          relu_done = 1'b0;
        end
      endcase
    end
    // relu_en = 0인 경우, data_out와 relu_done은 모두 0 (위에서 기본값으로 할당됨)
  end

endmodule