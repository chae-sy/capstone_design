...
<<<<<<< Updated upstream:Verilog/SPRNN_Top.sv
    // input buffer( R, G, B )
=======
    //Î≤ÑÌçº 2Ï∞®Ïõê?úºÎ°? ÎßåÎì§?ñ¥Ï§òÏïº?ï®. Í∑∏Î¶¨Í≥? shift ?ã†?ò∏ Î∞õÏ?ÎßêÍ≥† ?ïå?ïÑ?Ñú ?ïà?óê?Ñú count ?Ñ∏?Ñú shift?ïò?äî Í±∏Î°úÎ°?
>>>>>>> Stashed changes:Verilog/SPRNN_Top.v
    ...
    // (memory -> buffer) => PE array Ïó¨Í∏∞ÎèÑ ÏàòÏ†ïÌï¥ÏïºÌï®
...
>>>>>>> Stashed changes:Verilog/SPRNN_Top.v
    ...
    // (1,2,4) relu/(3) maxpool/(5) PE+adder == output buffer Ïó¨Í∏∞ Î∂ÄÎ∂Ñ ÌÉÄÏù¥Î∞ç Îã§Ïãú Í≥ÑÏÇ∞Ìï¥ÏïºÌï®.
    always_ff @( posedge clk or negedge rst_n) begin
        case(layer_num)
            3: begin 
                if (stage5_en) begin 
